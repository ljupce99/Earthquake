...
export default function StatsBar({ earthquakes }) {
  ...
  const stats = [
    ...
    { label: 'M4+ EVENTS',    value: strong,                color: strong > 0 ? 'var(--red)' : 'var(--text-dim)', sub: strong > 0 ? 'attention' : 'none' },
  ]
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1, background: 'var(--border)' }}>
      {stats.map(({ label, value, color, sub }) => (
        <div key={label} style={{
          ...
          <div style={{ fontSize: 9, color: 'var(--text-dim)', letterSpacing: '0.15em', marginBottom: 8 }}>{label}</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 32, fontWeight: 700, color, lineHeight: 1 }}>{value}</div>
          <div style={{ fontSize: 9, color: 'var(--text-dim)', letterSpacing: '0.1em', marginTop: 4 }}>{sub}</div>
        </div>
      ))}
    </div>
  )
